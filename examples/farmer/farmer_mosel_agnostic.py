...
def scenario_creator(
    scenario_name, use_integer=False, sense=pyo.minimize, crops_multiplier=1,
        num_scens=None, seedoffset=0, trying=False
):
    ...
    """
    # Define the model file and data file
    model_file = 'MOSEL/farmer_stochastic.mos'

    # Create a problem instance
    problem = xp.problem()

    # Load the Mosel model file
    problem.read(model_file)
    ...
    for crop in crops:
        print(f"Acres of {crop} to plant: {problem.getsol(f'Plant({crop})')}")

    print(f"Maximum expected profit: {problem.getobjval()}")
    quit()
    ...
    model.read("farmer.mos")

    # scenario specific data applied
    scennum = sputils.extract_num(scenario_name)
    assert scennum < 3, "three scenarios hardwired for now"
    model.setparam("RandomYield('wheat')", 2.0)
    quit()
    #y = model.getparam("RandomYield")
    if scennum == 0:  # below
        random_yield_values = {"wheat": 2.0, "corn": 2.4, "beets": 16.0}
        #y.set_values({"wheat": 2.0, "corn": 2.4, "beets": 16.0})
        
    elif scennum == 1: #average
        random_yield_values = {"wheat": 2.5, "corn": 3.0, "beets": 20.0}

    elif scennum == 2: # above
        random_yield_values = {"wheat": 3.0, "corn": 3.6, "beets": 24.0}
        #y.set_values({"wheat": 3.0, "corn": 3.6, "beets": 24.0})

    for (crop), value in random_yield_values.items():
        model.setparam(f"RandomYield({crop})", value)

    if trying:
        model.solve()
        profit = model.getObjVal()
        print(f"Maximum expected profit: {profit}")
        plant = model.getSolution("Plant")
        for crop, value in plant.items():
            print(f"Acres of {crop} to plant: {value}")
        quit()
...
def main():
    scenario_creator(scenario_name="Good2", use_integer=False, sense=pyo.minimize, crops_multiplier=1,
        num_scens=None, seedoffset=0, trying=True)
